...
questions. This deck answers only those seven. It deliberately does NOT
re-pitch the technology; docs/PRESENTATION_v2.pptx already does that, and
repeating it in front of the same panel wastes the slot.
...
def table(s, data, widths, top=2.1, left=0.85, total=11.65, rh=0.5, size=11.5):
    ...
    for r, row in enumerate(data):
        fill = ACCENT if r == 0 else (PANEL if r % 2 == 0 else RGBColor(0xFF, 0xFF, 0xFF))
        _rect(s, Inches(left), y, Inches(total), Inches(rh), fill,
              line=None if r == 0 else PANEL_LINE)
        x = left
        for c, cell in enumerate(row):
            cw = total * widths[c]
            colour = RGBColor(0xFF, 0xFF, 0xFF) if r == 0 else INK
            box(s, Inches(x + 0.14), y + Inches(0.09), Inches(cw - 0.24),
                Inches(rh), str(cell), size=size, colour=colour,
                bold=(r == 0), space=0)
            x += cw
        y += Inches(rh)
    return y
...
def bullets(s, items, top=2.1, size=17, gap=13, left=0.85, width=11.65):
    box(s, Inches(left), Inches(top), Inches(width), Inches(4.4), items,
...
def main():
    ...
    table(s, [
        ["The question you put to us", "Answered on"],
        ["Who is the customer, and how do you sell it?", "3"],
        ["How do you connect with the Sri Lanka Police?", "4, 5, 6"],
        ["At scale, where does the zone data come from?", "7, 8, 9, 10"],
        ["How do you import data from existing CCTV?", "11"],
        ["Revenue model - your cost, your earnings from one province", "12, 13, 14, 15"],
        ["SWOT analysis", "17"],
        ["Real-world implementation, and what will go wrong", "18, 19"],
    ], widths=[0.84, 0.16], top=2.1, rh=0.52)
    note(s, "We have not re-pitched the technology. You already said it works. "
            "This is how it reaches a road.", y=6.35)
    ...
    note(s, "The police are the biggest customer and the slowest customer. "
            "We cannot survive two years of procurement on no revenue - so we run two tracks.",
         y=6.5, size=14)
    ...
    rungs = [
        ("MONTH 0-3", "Give away evidence, not enforcement",
         "Free 30-day road-safety audit at one junction for one council. "
         "We issue ZERO fines. They get data they have never had. Nobody has to be brave to say yes."),
    ...
    y = 1.92
    for when, title, detail in rungs:
        ...
        y += 1.24
    note(s, "Each rung is something the other side can say yes to without "
            "spending money or taking a risk.", y=6.88, size=13.5)
    ...
    for i, (t, d) in enumerate([
        ...
        card(s, x, Inches(y), Inches(2.75), Inches(1.15))
        box(s, x + Inches(0.18), Inches(y + 0.12), Inches(2.4), Inches(0.4), t,
            size=14, colour=ACCENT, bold=True)
        box(s, x + Inches(0.18), Inches(y + 0.5), Inches(2.4), Inches(0.7), d,
    ...
    header(s, "Where the zone data comes from",
           "THE QUESTION YOU PRESSED HARDEST ON - AND YOU WERE RIGHT TO")
    ...
    box(s, Inches(0.85), Inches(3.35), Inches(11.65), Inches(0.5),
        "The answer starts with something our demo currently hides: there are TWO kinds "
        "of data, and they scale completely differently.", size=16, colour=ACCENT, bold=True)
    table(s, [
    ...
    note(s, "Our demo mixes the two, which makes the job look a hundred times bigger "
            "than it is. Separating them is the whole solution.", y=6.1, size=14)

    # ------------------------------------------------------------ 8  data 2
    s = S(); bg(s)
    header(s, "Step one: geo-register each camera once", "FOUR MINUTES, THEN NEVER AGAIN")
    box(s, Inches(0.85), Inches(2.0), Inches(11.65), Inches(0.6),
        ...
        colour=INK, space=0)
    items = [("Metres per pixel", "speed measurement"),
             ...
             ("Crossings, junction geometry", "future rules, free")]
    ...
    for i, (v, l) in enumerate([("4 min", "per camera, once"),
        ...
        stat(s, Inches(0.85 + i * 2.95), Inches(5.05), v, l, colour=ACCENT,
             w=Inches(2.75))
    note(s, "If a camera is knocked or moved, the static background changes - we detect it, "
            "stop enforcing the geometric rules and ask to be recalibrated.", y=6.6, size=13.5)

    # ------------------------------------------------------------ 9  data 3
    s = S(); bg(s)
    header(s, "Step two: import the world once, nationally",
    ...
    table(s, [
        ...
        ["Survey Dept / provincial GIS", "Built-up-area boundaries = the legal 50 km/h limit",
         "Government"],
        ["Gazetted traffic schemes", "No-parking, one-way, bus halts", "Municipal councils"],
        ["OpenStreetMap", "maxspeed, oneway, crossings, schools - urban coverage today", "Free"],
        ["Motor Traffic Act defaults", "Where nothing is known: the statutory limit for that road class",
         "The law itself"],
        ["Learned proposals", "'95% of vehicles here do 38-46 km/h - is the limit right?'",
         "Our own data"],
    ], widths=[0.26, 0.56, 0.18], top=2.05, rh=0.56, size=11)
    note(s, "The councils' schemes are paper and PDF today. Digitising Colombo's is a few weeks "
            "for one person - once, and then never again.", y=6.15, size=13.5)
    note(s, "Learned rules are PROPOSED for a human to approve. They are never auto-enforced.",
         y=6.7, size=13.5, colour=AMBER)

    # ------------------------------------------------------------ 10  refuse
    s = S(); bg(s)
    header(s, "And where we still do not know, we refuse to guess",
    ...
    note(s, "Six of ten rules work on day one with zero map data. We deploy immediately and "
            "enrich over weeks - the customer sees value in week one, not month six.",
         y=6.5, size=14)

    # ------------------------------------------------------------ 11  cctv
    s = S(); bg(s)
    header(s, "Cameras that are already on the poles", "CCTV INTEGRATION")
    box(s, Inches(0.85), Inches(2.0), Inches(11.65), Inches(0.55),
        "Practically every installed camera and NVR speaks **ONVIF / RTSP**. We pull a "
        "stream. No firmware change, no rewiring, nothing new on the pole.",
        size=15, colour=INK, space=0)
    table(s, [
        ...
        ["Detect at the edge, send EVENTS only",
         ...
         "Runs on what exists"],
    ], widths=[0.31, 0.48, 0.21], top=2.65, rh=0.64, size=11.5)
    box(s, Inches(0.85), Inches(4.72), Inches(11.65), Inches(0.5),
        "A 400x reduction in bandwidth - the difference between deployable and not.",
        size=17, colour=ACCENT, bold=True)
    for i, (t, d) in enumerate([
            ("One box, four cameras",
             "Sits in the existing cabinet. Runs on CPU today."),
            ("Survives the link dropping",
             "Events queue locally and sync when it returns."),
            ("We keep the frame, not the stream",
             "90-day retention, then auto-purge. PDPA by design.")]):
        x = Inches(0.85 + i * 3.92)
        card(s, x, Inches(5.05), Inches(3.72), Inches(1.15))
        box(s, x + Inches(0.2), Inches(5.18), Inches(3.4), Inches(0.45), t,
            size=13, colour=ACCENT, bold=True)
        box(s, x + Inches(0.2), Inches(5.6), Inches(3.4), Inches(0.6), d,
            size=11, colour=MUTED, space=0)
    note(s, "Honest caveat: most municipal CCTV was installed for theft, not enforcement. "
            "Expect ~40% of any estate to be usable without repositioning - we survey and say so "
            "before anyone signs.", y=6.4, size=13)
    ...
    table(s, [
        ...
        ["Provincial Licence", "Ministry / Police HQ. Unlimited cameras, ON-PREMISE, SLA, escrow",
    ...
    note(s, "The police will not put criminal evidence in someone else's cloud - and they "
            "should not. The on-premise Provincial Licence is our highest-margin product, "
            "not a compromise.", y=6.45, size=13.5)
    ...
    note(s, "Subscription pays us for coverage and deterrence - which is the outcome the "
            "customer actually wants. We think this is the mature part of our model.", y=6.4)

    # ------------------------------------------------------------ 14  unit economics
    s = S(); bg(s)
    header(s, "We price against the cost of a constable", "UNIT ECONOMICS")
    table(s, [
    ...
    box(s, Inches(0.85), Inches(5.25), Inches(11.65), Inches(0.7),
        ...
        bold=True)
    note(s, "And we are not replacing officers. We are telling them where to stand. "
            "That framing matters when the buyer has a union and a manpower shortage.",
    ...
    for i, (t, colour, items) in enumerate(quad):
    ...
    for i, (t, d) in enumerate([
        ...
        y = Inches(2.75 + i * 0.85)
        card(s, Inches(0.85), y, Inches(11.65), Inches(0.72))
        box(s, Inches(1.1), y + Inches(0.13), Inches(3.9), Inches(0.45), t,
            size=13.5, colour=ACCENT, bold=True)
        box(s, Inches(5.1), y + Inches(0.15), Inches(7.2), Inches(0.45), d,
            size=12, colour=INK, space=0)
